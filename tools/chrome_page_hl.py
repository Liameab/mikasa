        ...
        async with CDP(target["webSocketDebuggerUrl"]) as cdp:
            ...
            for g in tl["geoms"]:
                if not (0 < g["w"] <= 1.001 and 0 < g["h"] <= 1.001):
                    ...
                    break
            # 连续模式的文字层**不再断言"不该有"**：2026-09-11 起改成了按页窗口
            # 铺法（只铺当前页 ±2 页、滚过页边界同步），"不许铺满全部页面"这条
            # 不变量由 tools/chrome_textlayer.py 用 24 页素材守着——本工具的探针
            # PDF 只有 2 页，在这里断言规模没有意义（2026-09-16 全量排查发现：
            # 这条旧断言自那次改版起就一直在假失败，因为没人再跑过这个工具）。
