...
当前 **977 tests 全绿**，ruff + mypy clean，覆盖率 ~93%。已发布 Windows 免安装包与安装向导
（v0.1.0 → v0.1.9），应用内自带「检查更新 → 一键下载安装」——下载支持断点续传，
...
| v0.1.9 | **文生图**：问答页一键出图、随会话留存；出图独立成段（默认关）、上游图片当场落盘（链接 1 小时过期）、前端只渲染同源图片（远程图片是追踪像素）、出网两侧都过 SSRF 闸门（ADR-0031）；同批修好「载荷里模型收了两份」（CI 走 Xet 时 blobs/ 成第二副本，zip 白大一倍，ADR-0030 补丁） | ✅ 977 tests |
...
- 测试与覆盖率：**977 单测**，覆盖率 ~93%（见 evaluation.md 回归门禁）；
