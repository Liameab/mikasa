...
- **自动化评测三阶段**：A 检索层（recall/MRR/nDCG）→ B 生成协议层 →
  ...
  召回 recall@10=1.000、引用越界率 0、16 道不可答全部干净拒答。题库还能
  **照着你自己导入的资料现场出**，评测不再绑死一份示例语料（ADR-0026）；
- **跨语言检索**：中文提问自动译成英文作第二路查询，双路 RRF 合流；
  被引用的英文段落以「原文 + 中文翻译」对照呈现；
- **公式排版**：KaTeX 内置在发布物里（离线，不走 CDN），答案里的
  `$…$` / `$$…$$` 真的排出来——代码块与 `价格$5到$10之间` 不会被误判
  （ADR-0028）；
- **笔记与拍照入库**：知识库页左写右预览的 Markdown 笔记，保存即入库、
  可被引用、随时回来改；一张照片也能用视觉模型转成草稿笔记（识别只是
  草稿，结果可改，原图随笔记留存，ADR-0021 / ADR-0027）；
- **文生图**：问答页直接出图并随会话留存；上游图片当场落盘（链接只有
  1 小时有效期），前端只渲染同源图片（远程图片等于追踪像素），出网两侧
  都过 SSRF 闸门（ADR-0031）；
- **文档阅读视图**：点引用角标 → 侧栏打开原文，支持重建文本与 PDF
  原页渲染（引用区域高亮定位，与原文逐框对齐）；读到某处可以**就地问**
  （范围默认「本篇」、选中文字当上下文、命中片段全给且能跳回原文），
  即问即散不塞满会话树（ADR-0034）；
- **三套运行形态**：云端 API / 本地推理 / 零密钥离线，`--profile` 一键
  切换，代码零硬编码；
- **零构建链 Web**：四页原生 HTML/JS（问答 / 知识库 / 找论文 / 评测）+
  手写 SSE 流式问答；
- **会话管理树**：任意层嵌套文件夹（菜单移动、防环、删空夹），
  首问自动提炼标题（LLM 一次 / 截断兜底，手动命名永不被覆盖）——
  侧栏即工作台（ADR-0015）。
- **界面内配置模型**：设置面板选来源（本机 Ollama / DeepSeek / SiliconFlow /
  Claude / OpenAI / 任意 OpenAI 兼容服务，共六个预设）→ 贴密钥 → 测连通 →
  保存即生效，不用重启也不用改 `.env`；密钥只存本机数据目录、永不回显
  （ADR-0018 / ADR-0032）。选本机 Ollama 时还能**在应用内直接拉模型**看进度，
  不用再去命令行敲 `ollama pull`。
...
| `local` | Ollama qwen3（**原生 `/api/chat`**，免密钥；`think`/`num_ctx` 两个旋钮只有它认，ADR-0029） | fastembed bge-small-zh（512 维，**发布包随包携带**，ADR-0030）/ 暂无本地重排 | 全离线本地推理（见 ADR-0014） |
...
# 手机 / 平板 / 另一台电脑上打开浏览器用（v0.1.11 起，ADR-0033）：
mikasa auth set-password     # 先设访问口令——不开给网络时它不参与
mikasa serve --host 0.0.0.0  # 绑非回环地址
# 没设口令时这条命令会**拒绝启动**并告诉你先跑哪一条（fail closed）：
# 一旦开给网络，同网段的人都能读你的文档、花你的模型额度，所以宁可不开门也不裸奔。
# 本机浏览器与桌面端走回环地址，始终免口令；数据仍在各自机器上，不上云。
```
...
| `auth set-password` / `clear-password` | 设 / 清访问口令——`serve --host 0.0.0.0` 前必须先设（ADR-0033） |
...
| v0.1.11 | **浏览器/局域网访问 + 访问口令**（`mikasa auth set-password` + `serve --host 0.0.0.0`：回环免口令、非回环必须设口令否则拒绝启动；数据仍只在各自机器上，ADR-0033）；**修好应用内更新**（校验和改读 GitHub API 的 `asset.digest`——原来去 github.com 下那个小文件，国内握手被重置就整场更新失败、进度永远 0）；**阅读器「识别本页」**（PDF 页面视图一键识图，由 Codex 那条线实现、经真浏览器验收后合并） | ✅ 1018 tests |
| v0.1.12 | **阅读器「边看边问」**（阅读区底部常驻提问栏：范围默认「本篇」可切全库、选中文字当上下文、相关片段全给并可就地跳回原文高亮、即问即散不落库，ADR-0034）；**修好「每切一次来源就要重贴一次密钥」**（面板的"已保存"提示原来只看当前生效槽——改成按选中来源自己的密钥槽判断，密钥本来就没丢）；**修好本机档出题超时**（本地强制关思考 + 单次超时 120s，报错文案给出可照做的三步） | ✅ 1048 tests |
