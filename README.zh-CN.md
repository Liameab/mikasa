...
当前 **495 tests 全绿**，ruff + mypy clean，覆盖率 ~93%。M5（GitHub 发布）进行中。
...
问答置灰）、**知识库**页（网页上传/删除文档，入库即被检索；另有「在线找论文」
面板：检索 arXiv/OpenAlex 并把开放获取 PDF 直接导入语料库，导完即可提问——
知网式体验，不是知网的数据，付费墙全文拿不到）、**评测**页
...
- 测试与覆盖率：**430 单测**，覆盖率 ~93%（见 evaluation.md 回归门禁）；
