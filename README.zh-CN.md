...
当前 **815 tests 全绿**，ruff + mypy clean，覆盖率 ~93%。已发布 Windows 免安装包与安装向导
（v0.1.0 → v0.1.4），应用内自带「检查更新 → 一键下载安装」——下载支持断点续传，
...
中文开放获取期刊；带年份/语言/开放获取筛选与被引或时间排序；点一条看完整
摘要，导入开放获取 PDF 到语料库后即可提问，已导入的会标「已在库中」；**检索历史**一键重搜，详情里还有**相关论文 / 引用了它 / 参考文献**三个出口继续往下找。
...
- 测试与覆盖率：**815 单测**，覆盖率 ~93%（见 evaluation.md 回归门禁）；
