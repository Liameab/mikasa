...
| F3 | M6: knowledge base → notes | Four stages: **(1) write a note and have it searchable immediately — shipped 2026-09-16, ADR-0021** → **(2) photo upload converted to note text — shipped 2026-09-20, ADR-0027** → (3) LLM-generated knowledge links (mind the schema-migration discipline) → (4) knowledge map | Project notes | Stages 1 and 2 have landed; **stage 3 is foundation only** (2026-09-22: the `doc_links` table at schema v5 + a repo layer + migration tests; **the LLM extraction and the UI have not started, so the table will stay empty** — the user ruled on 2026-09-23 to archive it as unscheduled); stage 4 unscheduled |
