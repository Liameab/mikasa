...
> 面向"从零到会用"的完整使用文档：四个页面、查论文工作流、AI 提问细节用法、
...
## 2b. 手机 / 别的电脑用浏览器打开（局域网访问 + 口令）

Mikasa 本身就是个网页应用——桌面端只是给它套了个窗口。所以**同一 WiFi 下**，手机、
平板、另一台电脑都能用浏览器直接用（见 ADR-0033）。

**四条命令就够**：

```bash
# 1) 设一个访问口令（只存本机数据目录，加盐哈希、不存明文；至少 6 位）
mikasa auth set-password

# 2) 开给局域网（不带 --host 时默认只绑 127.0.0.1，只有本机能连）
mikasa serve --host 0.0.0.0

# 3) 记下启动横幅里的地址（形如 http://192.168.1.23:8000/），手机浏览器打开它
#    第一次会看到登录页，输入刚才那个口令即可

# 4) 换口令 / 取消口令
mikasa auth set-password      # 再设一次即更换（**所有已登录的设备会被踢下线**）
mikasa auth clear-password    # 取消后如果还绑着 0.0.0.0，下次启动会被拒绝
```

**几条要说清楚的**：

- **本机（这台电脑自己的浏览器）永远免口令**：能连到 `127.0.0.1` 的人本来就坐在你机器
  前面，没必要让自己天天输密码。
- **没设口令就开不了局域网**：`--host 0.0.0.0` 在没设口令时**直接拒绝启动**并告诉你
  怎么办——这是刻意的，否则同一 WiFi 下任何人都能读你的资料、删你的文档、
  还能用你配置的模型额度（密钥就存在服务端配置里）。
- **数据仍然只在你自己的电脑上**：手机只是"遥控器"，资料没有上传到任何地方，
  也没有第三方服务器。别人下载 Mikasa 装在自己电脑上，看到的是**他自己**的空库——
  每个人的数据天生独立。
- **局域网内是明文 http**：可信网络（家里、自己手机热点）没问题；不可信网络请别开。
...
（还有一页登录页 `/login`，只在浏览器从非本机地址访问时才出现，见 §2b。）

| 页 | 左侧 | 右侧 |
|---|---|---|
| 问答 | 会话管理树 | 聊天区（含自由问答开关） |
| 知识库 | 语料文件夹树 + 查找框 | 上传区 + 选中文档详情（阅读面板也在这里打开，含「边看边问」） |
| 找论文 | 检索历史 + 筛选/排序 | 结果列表 + 详情面板（导入 / 引证关系） |
| 评测 | 历史运行列表 | 黄金集跑评测 + 报告渲染 |
...
9. **就着正文提问（「边看边问」，v0.1.12 起）**——阅读区底部常驻一条提问栏，读到哪问到哪：**不用切回问答页，阅读位置不丢**。

   - **范围默认「本篇」**（只把这篇的块喂给模型），一键切「全库」：问"这段到底在说什么"用本篇，问"这跟别处怎么说的"再切全库；
   - 在正文里**选中一段文字**会随提问一起带进去（胶囊显示"已选中 N 字"，✕ 可去掉）——问"这段什么意思"最顺手；
   - 答案下面是**相关片段**列表：**本次命中的全都列出来**（不只答案引用的那几条），被引用那张带 `[n]` 角标；**点任意片段就地跳回原文并高亮**，属于别篇的标「另一篇」，点它阅读区就切过去；
   - **即问即散**：阅读器里的提问**不建会话、不进历史**（关掉就没了）——想留档请在问答页问；
   - 已知边界：**「原文件」标签里的 PDF 选不中文字**（那是浏览器自带阅读器，拿不到选区），要带上下文请在「文本」或「页面」标签里选。
...
- **笔记之间可以互链**：正文里写 `[[另一篇笔记的标题]]`（`[[标题|显示文字]]` 也可以），保存时
  这条链接就落进库里（`doc_links` 表）。代码块与行内代码里的 `[[…]]` 不算链接。
  - **改名会断链**：链接认的是标题，改名不会自动跟着改——重存一次那篇笔记即可。
  - **写了一半的链接不算错**：指向还没写的笔记名会被安静跳过。
  - 链接真正**参与检索**要开一个默认关的旋钮（`retrieval.hop_expand`，见 `config.example.yaml`）：
    开了之后，提问命中甲篇时会把**甲篇链到的乙篇**里最相关的那一块也带进资料。默认关着的理由
    写在 [ADR-0035](design-decisions.md) 里——它会平移评测基线，而且只有你的笔记真的互链了才有效。
...
- **来源预设**（六个）：Ollama 本机（免费离线免密钥）、DeepSeek（官方接口）、
  SiliconFlow（有免费模型，云端试用最省事）、**Claude**、**OpenAI**、自定义
  （任何 OpenAI 兼容服务：`/v1` 地址 + 模型名 + 密钥）。
  - **Claude** 走 Anthropic 官方的 **OpenAI 兼容入口**（`https://api.anthropic.com/v1/`，
    模型默认 `claude-opus-5`，可换 `claude-sonnet-5`）。如实说：官方把这层定位为兼容层，
    提示词缓存等原生特性用不上、不认识的参数会被忽略——能跑，但不是满血。
  - **OpenAI** 是官方接口（`https://api.openai.com/v1`）。**ChatGPT/Codex 订阅不能当
    API 用**：那不是 API，没有可粘贴的密钥。
- **本机模型**（选「Ollama 本机」时出现）：显示本机有没有这个模型，没有就点
  **「拉取模型」**——下载与校验都在应用内完成（带进度、可取消；取消后已下载的分层
  留在 Ollama 缓存里，再点会接着下）。**装 Ollama 本身仍要你自己装**（官网下载安装，
  装完托盘常驻）：我们不在应用里替你下 1.5GB 的安装器。
- **本机 vs 云端怎么选**：本机档的价值是**免费、离线、隐私**，不是"更强"——同一台
  8GB 显卡上，能跑的模型就那么大，质量上限摆在那里。**日常好用请走云端**；
  本机档留给"断网也能用""不想把资料发出去"这类场景。
...
| 现象 | 原因 / 解法 |
|---|---|
| 首次上传文档还要联网吗 | **不用**（v0.1.9 起）：中文向量模型已随安装包分发，解析→分块→向量→入库整条链路离线；源码版（pip install）仍会在第一次上网下载一次 |
| 「生成图片」要钱吗 / 会联网吗 | **会联网**（云端出图模型按张计费或耗免费额度，提示词会发到所选服务商）。默认是关的，要先去 ⚙ →「图像生成」接入；出图能力与识图各配各的（见 §6c 后的「生成图片」） |
| 生成的图存在哪 / 会进知识库吗 | 存在 `%LOCALAPPDATA%\Mikasa\generated\`，**不进知识库、不参与检索**；想让它可检索，用「识别图片」把它变成笔记正文 |
| 回答太短 / 想让它写长一点 | ⚙ →「模型」→「回答上限」调大（留空 = 跟随档位默认 4096）；本机档还可打开「思考模式」（更稳但慢得多，实测 1.6 秒 → 28 秒） |
