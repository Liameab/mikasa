...
- **未保存的修改有保护**：按 `Esc`、点取消或点遮罩都会先弹确认框；保存失败时窗口不关、正文不清空，
  补一下就能重试。
- **什么内容能被搜到**：正文、小标题、表格都能。**代码围栏里的内容不进检索**（markdown 解析器会整块
  跳过它，防止代码里的 `#` 被当成标题）——因此"整篇只有一段代码"的笔记无法保存（400）。希望某段代码
  能被检索到，就再写一句话描述它。
...
**找论文**是顶部导航里的独立一页（ADR-0020）。一次同时检索三个免费来源——arXiv（预印本，全部开放获取）、OpenAlex（期刊元数据，含中文期刊）与 CORE（开放获取聚合库，带全文链接）——搜到的论文可直接导进语料库，之后与普通文档一样能被提问。
...
- **中栏 · 结果**：中文或英文关键词都能搜。三个来源**并发**检索、结果**轮转出现**，一页 50 条（2026-09-19 由 20 提到 50），不会让某个来源把其余来源永远压在后面。每行给标题（点击 = 论文详情页）、来源徽标、作者、年份、期刊与被引数（该来源不提供时写明"该来源不提供"——0 次与"不知道"是两回事），外加两行摘要预览；**加载更多**翻下一页。
...
- **右栏 · 详情**：点任意一条看完整摘要与元数据，再点**导入知识库**下载 PDF 并入库。结果行与详情面板同时转成**已在库中**并给出**去提问** / **去知识库**两个出口；同一篇重复导入按内容哈希跳过。没有开放获取全文的那条，导入按钮提前禁用。
...
- 网络慢或下载失败：弹窗会如实说明，并留「打开发布页」从浏览器手动下载（永远的兜底）；
