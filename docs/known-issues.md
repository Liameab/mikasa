...
| F3 | M6「知识库 → 笔记库」 | 四期：**①记录/写笔记入库即被检索（2026-09-16 收工，ADR-0021）** → **②拍照上传转文本（2026-09-20 收工，ADR-0027）** → ③LLM 生成知识链（注意 schema 迁移纪律）→ ④知识导图 | memory 项目档案（用户亲述） | ①②已落地；**③只有地基**（2026-09-22：`doc_links` 表 schema v5 + repo 层 + 迁移测试；**LLM 抽取与界面未开工，表会一直是空的**——用户 2026-09-23 拍板留档待排期）；④待排期 |
