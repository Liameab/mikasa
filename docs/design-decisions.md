...
## ADR-0033 浏览器访问与访问口令：开给网络就必须设口令（fail closed）

**背景**：用户 2026-09-21 提出要做"浏览器版"（*"浏览器和软件都可以用，浏览器可以
实时更新，更方便"*），同时把边界说得很清楚：*"我希望每个人下载后的数据都是独立的，
而不是任何人可以看到我的资料"*。

两句合起来指向同一件事：**Mikasa 本来就是 Web 应用**（桌面端只是 pywebview 外壳），
"每人一份数据"在**本机安装**形态下天然成立（数据在各自的 `%LOCALAPPDATA%\Mikasa`）；
风险只出现在**把服务开给网络**的那一刻——一旦 `--host 0.0.0.0`，同一 WiFi 下**任何人**
都能读你的资料、删你的文档，还能花你的模型额度（密钥就存在服务端配置里）。

**决定**：

1. **浏览器/局域网访问是第一等入口**：`mikasa serve --host 0.0.0.0`（或配置
   `web.host`），手机/平板/别的电脑用浏览器打开服务地址即可用——四个页面、提问、
   知识库、找论文、评测全都能用（前端本来就是响应式的）。
2. **非回环绑定 = 必须设口令，没设就拒绝启动**（fail closed）：
   `mikasa serve --host 0.0.0.0` 在没设口令时直接退出，并给出能照做的命令
   （`mikasa auth set-password`）。宁可拒绝启动，也不默认裸奔。
3. **口令与会话**（全在标准库，不引依赖）：
   - 口令：PBKDF2-HMAC-SHA256（20 万轮 + 16 字节随机盐）写进
     `<数据目录>/auth.json`，**不存明文**；
   - 会话：**HMAC 签名的过期时间戳**（默认 30 天）——无服务端状态、无数据库表；
     **改口令即换会话密钥 → 所有旧会话立刻失效**（不需要额外的失效列表）；
   - **回环来源免口令**：本机浏览器与桌面端零摩擦——能连到 127.0.0.1 的人本来
     就已经坐在你机器上了。
4. **门的判据只有一条**：来源地址是不是回环。页面请求 302 去 `/login`（带 `next`
   回跳）、接口请求 401 JSON。登录页是自包含 HTML（不引任何静态资源——它们也在门后面）。
5. **不做账号体系**：本次只解决"别人看不到"。多用户（每人独立数据目录 + 注册登录）
   与云托管是另一个里程碑；用户明确要的"每人数据独立"在"各自装一份"的形态下已经成立。

**实测与踩坑**（2026-09-21）：

- **踩到一个"空门"**：门的开关读的是 `settings.web.host`，而 `--host` 只是 serve 里的
  一个局部变量——命令行开了 `0.0.0.0`、门却以为还在本机，于是**局域网地址直连拿到了
  200**（本机实测）。修法 = serve 把生效地址**写回 settings**，让横幅、门、uvicorn
  看到同一个值。教训：**同一个判据（"现在绑在哪儿"）有第二个消费点时必须回到同一处取值**。
- **本机走自己的局域网 IP 会被正确判成"非回环"**（不是 127.0.0.1），所以 E2E 能用
  "同一台机器 + 网卡地址"真实模拟局域网来源；`TestClient(client=(host, port))` 则能
  在单测里精确冒充任意来源地址。
- **真浏览器 E2E**（`tools/chrome_lan_login.py`）：绑 0.0.0.0 → 局域网地址打开 →
  落在登录页（且深链接带 `next`）→ 错口令 401 有提示 → 对口径后回到原深链接 →
  带会话的接口放行 → 已登录直开根页不再要口令 → console 零错误。

**代价与边界**：

- Cookie **不设 `Secure`**：局域网走的是明文 http，设了 Cookie 发不出去。要真正上公网，
  请放在 **HTTPS 反代后面**——注意那条路还有一件必须先做的事：**当前实现的判据是 TCP
  来源地址**，反代之后所有请求都来自代理（本机/内网）→ 会被当成回环放行。做云部署
  那一版时必须改成读 `X-Forwarded-For`（并只信自己那台代理）。
- 口令是**一把共享的钥匙**（谁拿到都能进），不是每人一个身份；要给"不同人不同权限"
  请等账号体系那一期。
- 局域网内仍是**明文 http**：同网段的抓包者能看到内容。可信网络（家里/自己手机热点）
  没问题；不可信网络请用云部署 + HTTPS。
- 门只挡 HTTP 层：**绑了非回环地址就等于把端口暴露给网络**——不要在不信任的网络里这么开。

**代码**：`web/auth.py`（口令/会话/`AuthGate`/登录页）、`web/routers/auth.py`（三端点）、
`web/app.py`（装门：最后加 = 最外层）、`cli/__init__.py`（`mikasa auth set-password` /
...
## ADR-0035 跨文档一跳：笔记 `[[互链]]` 落成 doc_links + 检索侧**默认关**的邻居扩展

- 状态：Accepted ｜ 2026-09-25（用户拍板"笔记互链 + 检索侧一跳，LLM 抽取仍留待排期"）
- 关联：ADR-0021（笔记 = 带标记的普通文档）、ADR-0011（RRF 只吃名次）、
  M6 ③ 的 `doc_links` 地基（schema v5）

**背景**：`doc_links` 表（schema v5）当初只留了**地基**——`replace_doc_links` /
`links_for_document` / 删除级联都写好了，但**没有任何写入方，表一直是空的**。
用户列的"四个缺失"里有"跨文档/多跳推理机制缺失：固定 top_n 检索无法保证跨文档
关联被正确处理"，指的就是这一块。原计划（M6 ③）是**LLM 读两篇文档判关系**，
用户 2026-09-23 已把它"留档待排期"——代价大（一遍文档对级调用）、质量不确定。

**决定**（前两条是用户拍板的取舍）：

1. **写入方先做确定性的一种：笔记里的 `[[标题]]` 互链**（`ingest/wikilinks.py`）。
   零 LLM、零联网、精度 100%——边是**用户自己写的**，没有"模型觉得这两篇相关"
   的噪声。与"知识库 → 笔记库"的方向一致：互链本来就是笔记应用的核心动作。
   LLM 抽取那一期落地时写同一张表、同一个入口，`source` 字段区分来源
   （这里写 `wikilink`），互不冲突。
2. **检索侧一跳扩展默认关**（`retrieval.hop_expand: 0`）。一开就会改变注入的
   片段集合，也就是**平移评测基线**——63 题那些数字得重跑才算数；而且它只在
   "你的笔记之间有互链"时才有效果，不是普适增益。要开就在 profile yaml 里给个正数。
3. **扩展的前提是"邻居的块本来就进了这次查询的候选"**（复用主查询已经算出来的
   融合排名，不重跑检索）。代价因此是零次额外搜索、零次额外嵌入调用；而早先
   那版"链过去就取邻居的第 1 名"是错的——BM25 对零词面命中的块也会返回
   （`top_k<=0` 的既有语义），那等于无条件往注入里灌一块无关内容，**引用协议
   还拦不住它**（它确实是一条编号资料）。测试当场把这条暴露了出来。
4. **追加在命中尾部、不参与重排、每个邻居只要一块、上限按文档数**。引用编号
   `[n]` 是**注入顺序**（generator 按 enumerate 编号），所以主命中必须稳定占据
   靠前的编号；上限按文档数是因为一篇热门笔记被十几篇链到时，只按块截断会让
   同一篇霸屏。

**为什么边是"标题"而不是 id**：`[[标题]]` 是人写笔记时唯一愿意敲的形式。
代价是**改名会断链**——解析不到就跳过（写了一半的笔记里 `[[还没写的笔记名]]`
是常态，那不是错误）。同名文档**全部落地**：个人库里重名多半是同一篇的两次
导入，取其一会在用户改了标题之后静默指向另一篇。

**代价与边界**：

- **改名断链**（见上）：改名不会自动修边，重存一次那篇笔记即可。
- **扩展进来的块是普通命中**：引用卡上分不出"这是链过来的"。刻意如此——
  它是**来源**，不是另一种东西；分标签会让引用协议多一个维度而收益不明。
- **`doc_links` 的入边不参与**：现在只有"我指向它"这一种关系，`links_for_document`
  两个方向都算邻居，所以互链是**双向可达**的。
- **LLM 抽取仍未做**（M6 ③ 的原计划）：本 ADR 只补上"另一半用法"，
  `known-issues.md` F3 的 ③ 状态随之更新。

**代码**：`ingest/wikilinks.py`（新，解析 + 落边）、`pipeline/retriever.py`
（`_neighbor_doc_ids` / `_expand_one_hop` + `oneside` 的 allow-list 参数化）、
`config/settings.py`（`RetrievalConfig.hop_expand`）、`config/config.example.yaml`、
